...
gene_names = []
df = pd.DataFrame(columns=['gene_name', 'Codon', 'AA', 'Count'])
data = []
...
for gene_info, gene_sequence in genes:
    ...
    for j in codons_x:
        for k in codons_y:
            count= count_TAA_j_followed_by_k(gene_sequence, j, k)
            a, b = amino_acids(j, k)
            data.append([gene_name, j+' '+k, a+' '+b, count])
df1 = pd.DataFrame(data, columns=['gene_name', 'Codon', 'AA', 'Count'])
...
for gene_info, gene_sequence in genes:
    ...
    for j in codons_x:
        for k in codons_y:
            count = count_TAG_j_followed_by_k(gene_sequence, j, k)
            a, b = amino_acids(j, k)
            data.append([gene_name, j+' '+k, a+' '+b, count])
df2 = pd.DataFrame(data, columns=['gene_name', 'Codon', 'AA', 'Count'])
...
for gene_info, gene_sequence in genes:
    ...
    for j in codons_x:
        for k in codons_y:
            count = count_TGA_j_followed_by_k(gene_sequence, j, k)
            a, b = amino_acids(j, k)
            data.append([gene_name, j+' '+k, a+' '+b, count])
df3 = pd.DataFrame(data, columns=['gene_name', 'Codon', 'AA', 'Count'])
df = df._append(df3, ignore_index=True)
df = df.groupby(['gene_name', 'Codon', 'AA']).agg({'Count': 'max'}).reset_index()
df = df.pivot(index='gene_name', columns=['Codon', 'AA'], values='Count').fillna(0)
df = df.fillna(0)
print(df)
df.to_csv('FS+1(All_seq_Count).csv')
